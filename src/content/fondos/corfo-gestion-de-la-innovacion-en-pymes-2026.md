---
title: "Corfo Gestión de la Innovación en Pymes 2026: fechas y requisitos"
seoTitle: "Corfo Gestión de la Innovación en Pymes 2026 · TomContable"
description: "Corfo cofinancia hasta $72.000.000 para que grupos de pymes incorporen innovación en su gestión. Revisa requisitos, plazo y el link oficial."
keyword: "corfo gestión de la innovación en pymes 2026"
institucion: "Corfo"
categoria: "Innovar"
montoMax: 7200000
resumen: "Gestión de la Innovación en Pymes es el cofinanciamiento de Corfo para que grupos de al menos 5 empresas (80% pymes) incorporen prácticas de innovación en su gestión. Corfo cubre hasta el 70% del proyecto, con tope de $7.200.000 por empresa y $72.000.000 por proyecto. La convocatoria 2026 está abierta hasta el 15 de julio."
fechaInicio: 2026-05-20
fechaCierre: 2026-07-15
beneficio: "Cofinanciamiento de hasta el 70% del proyecto, con tope de $7.200.000 por empresa y $72.000.000 por proyecto."
requisitos:
  - "Proyecto que agrupe al menos 5 empresas beneficiarias"
  - "Mínimo 80% de las empresas del grupo deben ser pymes"
  - "La postulación la presenta una entidad que organiza y ejecuta el proyecto, no una empresa sola"
link: "https://www.corfo.gob.cl/sites/cpp/programasyconvocatorias/"
fuenteTexto: "Ministerio de Economía, Fomento y Turismo"
fuenteHref: "https://www.economia.gob.cl/2026/05/22/corfo-abre-convocatorias-para-que-pymes-impulsen-sus-capacidades-de-innovacion-y-lideren-proyectos-id.htm"
pubDate: 2026-07-01
faqs:
  - q: "¿Para quién es este programa?"
    a: "Para grupos de al menos 5 empresas, donde un mínimo de 80% deben ser pymes, que quieren incorporar la innovación como práctica de gestión, no para una empresa que postula sola."
  - q: "¿Cuánto cofinancia Corfo?"
    a: "Hasta el 70% del costo total del proyecto, con un tope de $7.200.000 por empresa y $72.000.000 por proyecto completo."
  - q: "¿Hasta cuándo se puede postular?"
    a: "La convocatoria 2026 está abierta hasta el 15 de julio de 2026."
draft: false
---

Este programa no financia a una pyme sola: financia a un grupo de al menos 5 empresas que se juntan para trabajar la innovación como práctica de gestión, con talleres, metodologías y proyectos tecnológicos aplicados. Si tu pyme participa de un gremio, cámara o red de negocios, vale la pena revisarlo en conjunto.

## Requisitos mínimos

- El proyecto debe incluir al menos 5 empresas beneficiarias.
- Al menos el 80% de esas empresas deben ser pymes.
- La postulación la hace una entidad organizadora (gremio, cámara, consultora habilitada), no una empresa individual.

## Cómo postular

La postulación se hace en el sitio de Corfo. La convocatoria 2026 incluyó una charla informativa antes del cierre para resolver dudas sobre el armado del proyecto grupal.
