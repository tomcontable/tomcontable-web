---
title: "Capital Abeja Emprende 2026 (Sercotec): fechas, requisitos y postulación"
seoTitle: "Capital Abeja Emprende Sercotec 2026 · TomContable"
description: "Capital Abeja Emprende de Sercotec entrega $3.500.000 a mujeres emprendedoras para formalizar su negocio. Revisa requisitos, fechas y el link oficial."
keyword: "capital abeja emprende 2026"
institucion: "Sercotec"
resumen: "Capital Abeja Emprende es el fondo de Sercotec para mujeres con una idea de negocio que aún no formalizan actividades ante el SII. Entrega un subsidio no reembolsable de $3.500.000. La convocatoria 2026 estuvo abierta entre el 14 y el 27 de mayo, y ya cerró; Sercotec repite este llamado durante el año."
fechaInicio: 2026-05-14
fechaCierre: 2026-05-27
beneficio: "Subsidio no reembolsable de $3.500.000: entre $3.000.000 y $3.300.000 para inversión en activos, infraestructura y capital de trabajo, y entre $200.000 y $500.000 para gestión empresarial."
requisitos:
  - "Mujer, mayor de 18 años"
  - "Sin inicio de actividades en primera categoría ante el SII (idea o emprendimiento aún no formalizado)"
  - "Proyecto de negocio que se ajuste al foco definido por la convocatoria en su región"
  - "Sin deudas laborales, previsionales ni tributarias, y sin figurar en el Registro Nacional de Deudores de Pensiones de Alimentos"
link: "https://www.sercotec.cl/capital-abeja-emprende/"
fuenteTexto: "Sercotec, ficha del programa Capital Abeja Emprende"
fuenteHref: "https://www.sercotec.cl/programas/capital-abeja-emprende/"
pubDate: 2026-07-01
faqs:
  - q: "¿Para quién es Capital Abeja Emprende?"
    a: "Para mujeres mayores de edad con una idea de negocio o un emprendimiento informal, que todavía no hacen inicio de actividades en primera categoría ante el SII."
  - q: "¿Cuánto entrega el fondo?"
    a: "$3.500.000 no reembolsables, con la mayor parte destinada a inversión y una parte menor a gestión empresarial y formalización."
  - q: "¿Sigue abierta la convocatoria 2026?"
    a: "El llamado 2026 cerró el 27 de mayo. Sercotec suele repetir Capital Abeja Emprende en distintas regiones durante el año; revisa sercotec.cl/postulaciones-abiertas."
draft: false
---

Capital Abeja Emprende funciona igual que Capital Semilla Emprende, pero está reservado para mujeres. Es un subsidio no reembolsable que cofinancia un plan de trabajo para dejar un emprendimiento formalizado y funcionando, no un crédito que haya que devolver.

## Requisitos mínimos

- Mujer, mayor de 18 años.
- Sin inicio de actividades en primera categoría ante el SII.
- Proyecto de negocio que calce con el foco definido por Sercotec en la convocatoria de tu región.
- Sin deudas laborales, previsionales ni tributarias, y sin figurar en el Registro Nacional de Deudores de Pensiones de Alimentos.

## Cómo postular

Igual que en Capital Semilla Emprende, la postulación se hace en el sitio de Sercotec: registro de usuaria, lectura de las bases de tu región, test de preselección y video de presentación del proyecto. Las fechas varían según la región.
